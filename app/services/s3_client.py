...
class S3Service:
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                f"S3 client initialized for bucket '{self.bucket_name}' in region '{AWS_REGION}'."
            )
    ...
    async def upload_file(
        self, file: UploadFile, filename: str, content_type: str
    ) -> str:
        """
        ...
        if not file or not filename or not content_type:
            ...
            raise HTTPException(
                status_code=400, detail="Missing file, filename, or content type."
            )
        ...
        try:
            ...
            logger.error(
                f"Error uploading '{filename}' (key: {object_key}) to S3: {e}",
                exc_info=True,
            )
            raise HTTPException(
            ...
            logger.error(
                f"Unexpected error during upload of '{filename}' (key: {object_key}): {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
            ...
            try:
                ...
                logger.warning(
                    f"Error closing file object for {filename}: {close_err}",
                    exc_info=True,
                )
    ...
    def get_presigned_url(self, object_key: str, expiration: int = 3600) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error generating presigned URL for '{object_key}': {e}", exc_info=True
            )
            raise HTTPException(
            ...
            logger.error(
                f"Unexpected error generating presigned URL for '{object_key}': {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
                detail="An unexpected error occurred generating file access URL.",
            )
