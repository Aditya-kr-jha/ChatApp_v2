...
# --- Test Data ---
users_data = [
    {"username": "alice_test", "email": "alice@test.com", "first_name": "Alice", "last_name": "Tester", "password": DEFAULT_PASSWORD},
    {"username": "bob_test", "email": "bob@test.com", "first_name": "Bob", "last_name": "Check", "password": DEFAULT_PASSWORD},
    {"username": "charlie_test", "email": "charlie@test.com", "first_name": "Charlie", "last_name": "Debug", "password": DEFAULT_PASSWORD},
    {"username": "diana_test", "email": "diana@test.com", "first_name": "Diana", "last_name": "Script", "password": DEFAULT_PASSWORD},
    {"username": "eve_test", "email": "eve@test.com", "first_name": "Eve", "last_name": "Runner", "password": DEFAULT_PASSWORD},
]
    ...
    try:
        ...
        if hasattr(e, 'response') and e.response is not None:
            ...
            if e.response.status_code == 409:
                 print(f"  -> Note: Received 409 Conflict (maybe already exists/joined?) - {e.response.json().get('detail', e.response.text)}")
...
if not creator_token:
    print("Failed to authenticate channel creator. Aborting channel creation and joining.")
    exit()
...
for channel_info in channels_data:
    ...
    if response_data and "id" in response_data:
        ...
        print(f"Channel '{channel_info['name']}' created successfully with ID: {channel_id}")
...
for username, password in user_credentials.items():
    ...
    if not user_token:
        print(f"  Skipping channel joins for '{username}' due to authentication failure.")
        continue
    ...
    for channel_id in created_channel_ids:
        ...
        if response_data and response_data.get("status") == "conflict":
            print(f"  User '{username}' was already a member of channel {channel_id} (or owner).")
        elif response_data:
